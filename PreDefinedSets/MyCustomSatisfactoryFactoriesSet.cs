...
public class MyCustomSatisfactoryFactoriesSet : IFactoriesSet
{
    ...
    public void ApplyTo(BasePlan plan)
    {
        ...
        plan.AddFactory(new Factory("Завод Кабель-60") { Inputs = [new ResourceAmount("Wire", 120)], Outputs = [new ResourceAmount("Cable", 60)] });
        ...
        plan.AddFactory(new Factory("Завод Бетон-40") { Inputs = [new ResourceAmount("Limestone", 120)], Outputs = [new ResourceAmount("Concrete", 40)] });
