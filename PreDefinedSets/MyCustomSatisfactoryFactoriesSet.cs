...
public class MyCustomSatisfactoryFactoriesSet : IFactoriesSet
{
    ...
    public void ApplyTo(BasePlan plan)
    {
        ...
        plan.AddFactory(new Factory("Завод Рама-8") { Inputs = [new ResourceAmount("Reinforced Plate", 12), new ResourceAmount("Iron Rod", 48)], Outputs = [new ResourceAmount("Modular Frame", 8)] });
        ...
        plan.AddFactory(new Factory("Завод АрмЗалПлит-20") { Inputs = [new ResourceAmount("Iron Plate", 120), new ResourceAmount("Screw", 240)], Outputs = [new ResourceAmount("Reinforced Plate", 20)] });
